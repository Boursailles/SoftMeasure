...
class COMMANDS:
    ...
    def connection(self):
        ...
        I_max = max(abs(float(self.settings['I_start'])), abs(float(self.settings['I_stop'])))
        assert I_max < self.instr.I_max, f"The supplied current (I = {I_max} A) is higher than the maximal one (I = {self.instr.I_max} A) supported by the device {self.settings['device']}."
